...
const sortOptions: { value: ProductOrdering | ""; label: string }[] = [
  { value: "", label: "پیش‌فرض" },
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <fieldset className="m-0 flex flex-col gap-2 border-0 p-0">
        <legend className="mb-2 p-0 text-small font-semibold">دسته‌بندی</legend>
      ...
      <fieldset className="m-0 flex flex-col gap-3 border-0 border-t border-gray-100 p-0 pt-6">
        <legend className="mb-2 p-0 text-small font-semibold">قیمت</legend>
        <PriceRangeSlider
          min={PRICE_MIN}
          max={PRICE_MAX}
          value={priceRange}
      ...
      <fieldset className="m-0 flex flex-col gap-2 border-0 border-t border-gray-100 p-0 pt-6">
        <legend className="mb-2 p-0 text-small font-semibold">موجودی</legend>
        <Checkbox label="فقط کالاهای موجود" checked={inStockOnly} onChange={(e) => onInStockChange(e.target.checked)} />
      </fieldset>
    </div>
  );
}
  ...
  return (
    <div className="mx-auto max-w-page px-5 xl:px-10">
      ...
      <div className="flex flex-col gap-4 pb-10">
        <h1 className="m-0 text-h1 font-bold">همه محصولات</h1>
        <p className="m-0 max-w-text text-body-large text-gray-800">
          {allProducts.length} قطعه، همه در کارگاه خودمان چاپ و بازرسی می‌شوند. مشخصات فنی هر قطعه کامل ذکر شده است.
        </p>
      </div>
          ...
          <div className="flex flex-col gap-4 border-b border-gray-100 pb-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <span className="text-small text-gray-800">
                {data ? `${data.results.length} محصول از ${data.count}` : "…"}
              </span>
              {hasActiveFilters && (
                <div className="flex flex-wrap gap-2">
                  ...
                  {inStock && <Tag onRemove={() => updateParams({ inStock: undefined })}>فقط موجود</Tag>}
                </div>
              )}
      ...
      <BottomSheet open={filtersOpen} onClose={() => setFiltersOpen(false)} title="فیلترها">
        <FiltersPanel {...filtersPanelProps} />
      </BottomSheet>
    </div>
  );
