  ...
  return (
    <div className="mx-auto max-w-page px-5 xl:px-10">
      ...
      <div className="flex flex-col gap-4 pb-10">
        <h1 className="m-0 text-h1 font-bold">دسته‌بندی‌ها</h1>
        <p className="m-0 max-w-text text-body-large text-gray-800">
          {categories.length} دسته، {allProducts.length} قطعه. تقسیم‌بندی بر اساس جایی است که قطعه استفاده می‌شود، نه
          شکل آن.
        </p>
      </div>
      ...
      <section className="grid grid-cols-1 gap-6 pb-14 md:grid-cols-2 md:pb-20 lg:grid-cols-3">
        {categoriesWithCounts.map(({ category, count }) => (
          ...
          >
            <ImagePlaceholder
              caption={category.slug.toUpperCase().replace(/-/g, " ")}
              className="aspect-[16/11] tracking-[0.08em]"
            />
      ...
      <section className="mb-14 grid grid-cols-1 gap-8 rounded-xl bg-graphite p-6 text-fog-white md:mb-20 md:p-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:items-center">
        <div className="flex flex-col gap-4">
          <p dir="ltr" className="m-0 font-mono text-micro tracking-[0.08em] text-titanium">
            MOST BUILT &middot; {featuredCategory.category.slug.toUpperCase()}
          </p>
          <h2 className="m-0 text-h2 font-semibold">{featuredCategory.category.name}، پرکارترین دسته</h2>
          ...
          >
            {featuredCategory.count} محصول {featuredCategory.category.name}
          </Link>
        </div>
      ...
      <section className="flex flex-col gap-8 border-t border-gray-100 py-14 md:py-20">
        <div className="flex flex-wrap items-baseline justify-between gap-4">
          <h2 className="m-0 text-h2 font-semibold">پرفروش هر دسته</h2>
          <Link
            to="/products"
