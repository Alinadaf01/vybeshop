...
import { getSearchConsolePages } from "@/lib/api";
...
export function PagesCard({ from, to }: { from: string; to: string }) {
  const { data, isPending, isError, refetch } = useQuery({
    queryKey: ["search-console-pages", from, to],
    queryFn: () => getSearchConsolePages({ from: from || undefined, to: to || undefined }),
  });

  return (
    <ReportSection title="صفحات برتر" description="صفحاتی که بیشترین نمایش و کلیک را در جست‌وجوی گوگل داشته‌اند.">
      {isPending ? (
        <ReportLoading />
      ) : isError ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-start text-sm">
            ...
            <tbody className="divide-y divide-white/[0.04]">
              {data.map((row) => (
                <tr key={row.page}>
                  <td className="max-w-xs truncate px-3 py-2 font-semibold text-white" dir="ltr" title={row.page}>
                    {row.page}
                  </td>
